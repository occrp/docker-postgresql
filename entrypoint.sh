...
if [ -s /var/lib/postgresql/PG_VERSION ]; then
    ...
    if [ ! -s /etc/postgresql/postgresql.conf ]; then
        ...
        if pg_createcluster -l /var/log/postgresql/ -s /var/run/postgresql/ -d /var/lib/postgresql 9.4 main >/dev/null 2>&1; then
            echo "...success."
            echo
            echo "* * * NOTICE: *not* setting listen_addresses to '*' as we're using an existing config * * *"
            echo "*   if you need network connectivity (like other containers connecting to postgres via    *"
            echo "*   TCP/IP), you have to inspect and update the config manually, if needed                *"
            echo "* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *"
            echo
            ...
            # set the settings
            # *NOT* setting the listen_addresses, as we're using an existing config!
            sed -i -r\
